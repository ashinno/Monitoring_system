...
def run():
    ...
    users = ["admin", "alice", "bob", "system", "unknown", "charlie", "dave", "eve"]
    ...
    for i in range(64):
        ...
        try:
            ...
            if res.status_code == 200:
                print(f"[{i+1}/64] Created log: {log['description']}")
            else:
                print(f"[{i+1}/64] Failed: {res.text}")
        except Exception as e:
            print(f"Error creating log: {e}")
        
        # Faster generation to avoid long wait
        time.sleep(0.1)
