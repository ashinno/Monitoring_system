    ...
    for i in range(100):
        ...
        try:
            ...
            if res.status_code == 200:
                # Print only every 10th log to reduce clutter
                if (i+1) % 10 == 0:
                    print(f"[{i+1}/100] Created log: {log['description']}")
            else:
                print(f"[{i+1}/100] Failed: {res.text}")
        except Exception as e:
            print(f"Error creating log: {e}")
        
        # Minimal delay for fast generation
        time.sleep(0.05)
